...
def main():
    ...
    for key, value in results.items():
        print(f"{key}: {value}")
    """
    moments_test_signal, status_array, signal = Piecewise_Signal_Experiment_Tests(
        frequency,
        variances,
        length,
        sampling_rate,
    ...
    binary_values = [0 if status == "Stationary" else 1 for status in status_array]
    ...
    plt.title("Binary Representation of Stationarity Status")
